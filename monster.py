...
async def notify(uid, text):
    """يرسل رسالة إشعار منفصلة للمستخدم (لا تُخرب أي واجهة قائمة)."""
    if not bot_app:
        return
    try:
...
# ============== منطق اللعبة (سريع: يستخدم التوكن المحفوظ أولاً دائمًا) ==============

async def get_monster(aid, ahash, sess, tok=None):
    def pick_real_monster(monsters_list):
        # يفضّل أول وحش حقيقي (غير بيضة)، وإلا يرجع أول عنصر كحل احتياطي
...
# ============== الخلفية (تعمل لكل الحسابات، حتى غير النشط منها حاليًا) ==============
                ...
                for a in d["accs"]:
                    ...
                    if target:
                        ...
                        if a.get("sched", 0) == 0:
                            ...
                            status = None
                            if a["noads"]:
                                status = await buy_direct(a["tok"], mid, it)
                            elif a["ads"]:
                                async with aiohttp.ClientSession() as s:
                                    status = await buy_with_ad(s, a["tok"], mid, it)
                                    if status == 401:
                                        new_tok = (await get_monster(a["aid"], a["ahash"], a["sess"]))[3]
                                        if new_tok:
                                            a["tok"] = new_tok
                                            status = await buy_with_ad(s, new_tok, mid, it)
                            ...
                            if a.get("notify", True):
                                ...
                                if status == 200:
                                    ...
                                    await notify(uid, f"⚠️ فشل شراء **{label}** لحساب **{a['name']}**. السبب: كود `{status}`")
...
async def on_creds(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    a = {"aid": aid, "ahash": ah, "sess": sess, "tok": tok, "name": m.get("name", "وحش"),
         "mid": m.get("_id"), "ads": False, "noads": False, "th": 55, "sched": 0, "sv": None, "notify": True}
    d["accs"].append(a)
    d["idx"] = len(d["accs"]) - 1
...
async def on_button(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("d_"):
        ...
        st = await buy_direct(a["tok"], a["mid"], item)
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == "--child":
        ...
        while True:
            print("🚀 تشغيل عملية جديدة للبوت...")
            result = subprocess.run([sys.executable, __file__, "--child"])
            print(f"\n⚠️ توقفت العملية (كود الخروج: {result.returncode}) — إعادة تشغيل خلال 10 ثوانٍ...\n")
            time.sleep(10)